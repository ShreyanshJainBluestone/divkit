// Generated code. Do not modify.

#[allow(unused_imports)]
use serde::{Serialize, Deserialize};
#[allow(unused_imports)]
use crate::types::*;
#[allow(unused_imports)]
use crate::generated::*;


#[derive(Serialize, Deserialize, Debug, Clone, PartialEq)]
pub struct BooleanVariable {
    pub name: String,
    #[serde(default = "BooleanVariable::default_type")]
    pub r#type: String,
    pub value: Expression<bool>,
}

#[derive(Debug, Clone, Default)]
pub struct BooleanVariableBuilder {
    pub name: Option<String>,
    pub r#type: Option<String>,
    pub value: Option<Expression<bool>>,
}

impl BooleanVariable {
    pub fn builder() -> BooleanVariableBuilder {
        BooleanVariableBuilder::default()
    }
    pub fn default_type() -> String { "boolean".to_string() }
}

impl BooleanVariableBuilder {
    pub fn name(mut self, value: String) -> Self {
        self.name = Some(value);
        self
    }
    pub fn r#type(mut self, value: String) -> Self {
        self.r#type = Some(value);
        self
    }
    pub fn value(mut self, value: Expression<bool>) -> Self {
        self.value = Some(value);
        self
    }
    pub fn build(self) -> BooleanVariable {
        BooleanVariable {
            name: self.name.expect("missing required field 'name'"),
            r#type: String::from("boolean"),
            value: self.value.expect("missing required field 'value'"),
        }
    }
}
